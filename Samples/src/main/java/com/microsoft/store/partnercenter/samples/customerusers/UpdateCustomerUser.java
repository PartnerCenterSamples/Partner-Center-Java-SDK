...
public class UpdateCustomerUser extends BasePartnerScenario {
    ...
    /**
     * executes the update customer user scenario by creating a customer and then updating it.
     */
	@Override
	protected void runScenario() {
        ...
        CustomerUser createdCustomerUser = partnerOperations.getCustomers().byId(selectedCustomerId).getUsers().create( userToCreate );
        ...
        customerUserToUpdate.setUserPrincipalName( UUID.randomUUID().toString().toUpperCase() + "@" + selectedCustomer.getCompanyProfile().getDomain() );
        
        // Now update the newly created customer user
        this.getContext().getConsoleHelper().startProgress( "Created user. Updating the user" );
        CustomerUser updatedCustomerUser = partnerOperations.getCustomers().byId( selectedCustomerId ).getUsers().byId( createdCustomerUser.getId() ).patch( customerUserToUpdate );
